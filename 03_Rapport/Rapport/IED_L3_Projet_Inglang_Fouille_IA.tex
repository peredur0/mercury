...
def create_document(mail, categorie):
    ...
    return doc
			...
			\paragraph{Stockage des données: ElasticSearch}
				...
				Il est cependant assez compliquer de modifier un index une fois qu'il a été créé. Pour ces raisons, j'utilise cette technologie pour ne stocker les données textuelles après nettoyage car elles n'ont plus vocation à être modifier. \\
				Les corps de mail mise en base seront récupérés ultérieurement pour réaliser les opérations d'analyse. Les résultats seront stockés dans la base PostgreSQL. 
