...
\section{Introduction}
	Ce projet à pour but de permettre de faire la détection des spam et ham dans des mails.
	...
	\end{figure}
	La \emph{Phase 1} concerne la récolte des informations et les traitements minimum nécessaire pour la mise en base. Les objectifs de traitement de cette phase sont:
	\begin{itemize}
		...
		\item Eliminer les parties de textes non pertinente (liens, réponses, certaines ponctuation)
	\end{itemize}
	Cette phase se termine avec la mise en base des documents dans un index ElasticSearch.\\
	
	La \emph{phase 2} vise à générer les données statistiques et numériques à partir des mails pré-traité. C'est a cette étape que qu'interviennent les 
...
\section{Récolte des données}
	...
	\subsection{Recherche de dataset}
		...
		J'ai pu alors récupéré deux dataset:
		\begin{itemize}
		...
		% Les mails Enron seront utilisés après le développement pour ajouter des données complémentaires une fois le développement terminer. 
...
\section{Pré-traitement}
	...
	Il va être possible d'effectuer certains traitement de nettoyage et de récupération d'informations sommaires.
	
	\subsection{Extraction des corps des mails}
		L'extraction du corps du mail se fait avec le module natif de python \emph{email}. \\
